...
```
ss -lntH | grep -Eq "127\\.0\\.0\\.1:${puerto}([[:space:]]|$)|\\[::1\\]:${puerto}([[:space:]]|$)"
```
...
```
ss -lntH | grep -Eq "0\\.0\\.0\\.0:${puerto}([[:space:]]|$)|\\[::\\]:${puerto}([[:space:]]|$)|\\*:${puerto}([[:space:]]|$)"
```
...
if ip -4 address show dev tun0 2>/dev/null | grep -q '10.100.0.1/24'; then
...
if compgen -G '/var/backups/infdemic/infdemic_*.tar.gz' > /dev/null; then
